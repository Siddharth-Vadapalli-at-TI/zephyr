...
/* Bit position of the ASEL field within a 64-bit PKTDMA/CPPI5 address */
#define AM62L_ADDRESS_ASEL_SHIFT   48U

/* The 4 bits corresponding to ASEL in a 64-bit address */
#define AM62L_ADDRESS_ASEL_MASK    GENMASK64(51, 48)

/*
 * ASEL values 14 and 15 route DMA transactions through the ARM Accelerator
 * Coherency Port (ACP).  ACP transactions are fully coherent with the CPU
 * cache, so no explicit cache flush or invalidate is required for DMA
 * buffers and descriptors on coherent channels.
 */
...
__subsystem struct ti_pktdma_driver_api {
	...
	/**
	 * @brief Diagnostic: dump completion ring interrupt register state
	 *
	 * @param dev     PKTDMA device
	 * @param chan_id PKTDMA channel number
	 */
	void (*ring_irq_dump)(const struct device *dev, uint32_t chan_id);

	/**
	 * @brief Diagnostic: dump CHANRT + RINGRT full state
	 *
	 * @param dev     PKTDMA device
	 * @param chan_id PKTDMA channel number
	 */
	void (*chan_dump)(const struct device *dev, uint32_t chan_id);
};
 ...
 * @brief Diagnostic: dump completion ring interrupt register state
 */
static inline void ti_pktdma_ring_irq_dump(const struct device *dev,
					   uint32_t chan_id)
{
	const struct ti_pktdma_driver_api *api =
		(const struct ti_pktdma_driver_api *)dev->api;

	api->ring_irq_dump(dev, chan_id);
}

/**
 * @brief Diagnostic: dump full CHANRT + RINGRT state
 */
static inline void ti_pktdma_chan_dump(const struct device *dev,
				       uint32_t chan_id)
{
	const struct ti_pktdma_driver_api *api =
		(const struct ti_pktdma_driver_api *)dev->api;

	api->chan_dump(dev, chan_id);
}
