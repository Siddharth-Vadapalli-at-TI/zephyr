		      ...
		      struct dma_config *cfg)
{
	...
	sys_write32(chanrt + PKTDMA_CHANRT_TCFG, tcfg);

	if (!pcfg->is_rx) {
		/* TX: set completion queue (TCQ = tflow_id = chan_id). */
		sys_write32(chanrt + PKTDMA_CHANRT_TCQ, chan_id);
	} else {
		/* RX: configure flow RT (EPIB + PSD present) */
		sys_write32(fwd_rt + PKTDMA_RX_FLOWRT_RFA,
...
static int pktdma_start(const struct device *dev, uint32_t chan_id)
{
	...
	sys_write32(ch->chanrt + PKTDMA_CHANRT_CTL, 0U);
	sys_write32(ch->chanrt + PKTDMA_CHANRT_CTL,
	...
	sys_write32(ch->fwd_rt + RINGACC_RT_INT_OFS,
			    ...
			    uint64_t desc_phys)
{
	...
	sys_write32(ch->fwd_rt + RINGACC_RING_RT_DB, 1U);
			   ...
			   uint64_t *desc_phys)
{
	...
	*desc_phys = ch->rev_mem[ch->rev_ridx] & ~AM62L_ADDRESS_ASEL_MASK;
	...
	sys_write32(ch->rev_rt + RINGACC_RING_RT_DB, (uint32_t)(-1));
...
static void pktdma_ring_irq_arm(const struct device *dev, uint32_t chan_id)
{
	...
	sys_write32(data->chans[chan_id].fwd_rt + RINGACC_RT_INT_OFS,
		    RINGACC_RT_INT_ENABLE_COMPLETE_TR);
}
...
static void pktdma_ring_irq_clear(const struct device *dev, uint32_t chan_id)
{
	...
	sys_write32(data->chans[chan_id].fwd_rt +
