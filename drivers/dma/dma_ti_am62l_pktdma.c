...
/**
 * @brief Dump ring interrupt register state for a channel (diagnostic)
 *
 * @param dev     PKTDMA device
 * @param chan_id PKTDMA channel ID
 */
static void pktdma_ring_irq_dump(const struct device *dev, uint32_t chan_id)
{
	struct pktdma_data *data = dev->data;
	struct pktdma_chan *ch;
	...
	LOG_DBG("PKTDMA chan %u INT regs (fwd_rt=0x%08x):", chan_id,
		(uint32_t)ch->fwd_rt);
	LOG_DBG("  enable_set  = 0x%08x (want 0x5)",
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS));
	LOG_DBG("  clr         = 0x%08x",
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS + RINGACC_RT_INT_CLR_OFS));
	LOG_DBG("  status_set  = 0x%08x",
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS + RINGACC_RT_INT_STATUS_SET_OFS));
	LOG_DBG("  status      = 0x%08x (non-zero = pending)",
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS + RINGACC_RT_INT_STATUS_OFS));
	LOG_DBG("  fwd ring OCC = %u  rev ring OCC = %u",
		pktdma_read32(ch->fwd_rt + RINGACC_RING_RT_OCC),
		pktdma_read32(ch->rev_rt + RINGACC_RING_RT_OCC));
}

/**
 * @brief Dump CHANRT + RINGRT state for a channel (diagnostic)
 *
 * @param dev     PKTDMA device
 * @param chan_id PKTDMA channel ID
 */
static void pktdma_chan_dump(const struct device *dev, uint32_t chan_id)
{
	struct pktdma_data *data = dev->data;
	struct pktdma_chan *ch;
	uintptr_t chanrt;
	...
	LOG_DBG("PKTDMA chan %u dump:", chan_id);
	LOG_DBG("  CHANRT CTL  (0x%08x) = 0x%08x",
		(uint32_t)(chanrt + PKTDMA_CHANRT_CTL),
		pktdma_read32(chanrt + PKTDMA_CHANRT_CTL));
	LOG_DBG("  CHANRT TCFG (0x%08x) = 0x%08x",
		(uint32_t)(chanrt + PKTDMA_CHANRT_TCFG),
		pktdma_read32(chanrt + PKTDMA_CHANRT_TCFG));
	LOG_DBG("  CHANRT TCQ  (0x%08x) = 0x%08x",
		(uint32_t)(chanrt + PKTDMA_CHANRT_TCQ),
		pktdma_read32(chanrt + PKTDMA_CHANRT_TCQ));
	LOG_DBG("  fwd_rt=0x%08x  enable_set=0x%08x  status=0x%08x",
		(uint32_t)ch->fwd_rt,
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS),
		pktdma_read32(ch->fwd_rt + RINGACC_RT_INT_OFS + RINGACC_RT_INT_STATUS_OFS));
	LOG_DBG("  fwd OCC=%u  rev OCC=%u",
		pktdma_read32(ch->fwd_rt + RINGACC_RING_RT_OCC),
		pktdma_read32(ch->rev_rt + RINGACC_RING_RT_OCC));
}
...
static DEVICE_API(ti_pktdma, pktdma_driver_api) = {
	...
	.ring_irq_dump  = pktdma_ring_irq_dump,
	.chan_dump      = pktdma_chan_dump,
};
