			      ...
			      void *user_data)
{
	...
	uint32_t port_num, port_base, macsl_ctl;
 ...
 *
 * @param config Pointer to CPSW Device-Tree configuration
 * @param dma Pointer to the PKTDMA state
 ...
 */
static void cpsw_tx_fill_chain(const struct cpsw_dt_config *config,
			       struct cpsw_pktdma *dma, uint32_t head_idx,
...
static int cpsw_tx(const struct device *dev, struct net_pkt *pkt)
{
	...
	cpsw_tx_fill_chain(config, dma, head_idx, pkt, pkt_len,
			   port_num, n_descs, needs_pad);
