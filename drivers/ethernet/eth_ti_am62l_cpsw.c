		      ...
		      uint16_t n_frags, uint16_t pkt_len)
{
	...
	cppi5_fill_tx_desc(head_desc, (uintptr_t)frags[0]->data, frags[0]->len,
			   port_num, config->tx_asel, config->tx_chan_id);
	head_desc->pkt_info0 =
	...
	for (i = 1; i < n_frags; i++) {
		...
		cppi5_fill_buf_desc(desc, (uintptr_t)frags[i]->data,
				    frags[i]->len, next_phys, config->tx_asel);
		if (!AM62L_PKTDMA_IS_COHERENT(config->tx_asel)) {
	...
	if (n_frags > 1) {
		...
		head_desc->next_desc =
			...
			FIELD_PREP(AM62L_ADDRESS_ASEL_MASK, (uint64_t)config->tx_asel);
	}
...
static int cpsw_init(const struct device *dev)
{
	...
	LOG_DBG("CPSW: TX IRQ %d RX IRQ %d connected",
