 ...
 * @param mac_cfg    MAC Address configuration structure
...
static int cpsw_init(const struct device *dev)
{
	 ...
	 * fallback to the MAC eFuse registers.
	 */
	ret = net_eth_mac_load(&config->mac_cfg, priv->mac_addr[0]);
	if (ret == -ENODATA) {
		/* Fallback to MAC eFuse registers */
		uint32_t mac_lo = sys_read32(priv->mac_efuse_base);
		uint32_t mac_hi = sys_read32(priv->mac_efuse_base +
					     CPSW_MAC_LO_MAC_HI_OFFSET);
		uint8_t *mac = priv->mac_addr[0];
