...
static int cpsw_mdio_init(const struct device *dev)
{
	...
	size_t reg_size, map_size;
	uintptr_t phys, page_off;
	mm_reg_t base, page_virt;
	uint16_t retries;

	phys = DEVICE_MMIO_ROM_PTR(dev)->phys_addr;
	page_off = phys & (CONFIG_MMU_PAGE_SIZE - 1U);
	...
	if (page_off == 0U) {
		...
		reg_size  = DEVICE_MMIO_ROM_PTR(dev)->size;
		map_size  = ROUND_UP(page_off + reg_size, CONFIG_MMU_PAGE_SIZE);

		device_map(&page_virt, phys - page_off, map_size,
			   K_MEM_CACHE_NONE);
		*DEVICE_MMIO_RAM_PTR(dev) = page_virt + page_off;
	}
	...
	if (mdio_readreg(base, MDIO_ALIVE) == 0U) {
		LOG_WRN("MDIO ALIVE=0 - no PHY responded; check PHY power, "
			"MDIO/MDC wiring, and DT PHY addresses vs PHYAD strapping");
	}
